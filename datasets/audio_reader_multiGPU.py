...
Function: Reading audios for multiple GPU version
...
def make_batches(wav_sorted_list,con_sorted_list,batch_size,seq_len,con_frame_size,con_dim,sample_rate,gpu_num,gpu_index):
    ...
    batch_size=min(batch_size,np.int32(np.ceil(len(wav_sorted_list)/float(gpu_num))))

    wav_sorted_list0=[]
    con_sorted_list0=[]
    for i in xrange((len(wav_sorted_list)-1)/gpu_num+1):
        wav_sorted_list0.append(wav_sorted_list[i*gpu_num])
        con_sorted_list0.append(con_sorted_list[i*gpu_num])

    if gpu_index==0:
        wav_sorted_list=wav_sorted_list0
        con_sorted_list=con_sorted_list0
    else:
        wav_sorted_list_index=[]
        con_sorted_list_index=[]
        for i in xrange((len(wav_sorted_list)-gpu_index-1)/gpu_num+1):
            wav_sorted_list_index.append(wav_sorted_list[gpu_index+i*gpu_num])
            con_sorted_list_index.append(con_sorted_list[gpu_index+i*gpu_num])
        wav_sorted_list=wav_sorted_list_index
        con_sorted_list=con_sorted_list_index

        total_len=0
        for i in xrange((len(wav_sorted_list0)-1)/batch_size+1):
            audio, _ = librosa.load(wav_sorted_list0[i*batch_size].split()[0], sr=sample_rate, mono=True)
            condition=ReadFloatRawMat(con_sorted_list0[i*batch_size].split()[0],1).reshape(1,-1)
            if len(audio)>condition.shape[1]/con_dim*con_frame_size:
                total_len+=__round_to(condition.shape[1]/con_dim*con_frame_size,seq_len)
            elif len(audio)<condition.shape[1]/con_dim*con_frame_size:
                total_len+=__round_to((condition.shape[1]/con_dim-1)*con_frame_size,seq_len)
            else:
                total_len+=len(audio)
    ...
    for i in xrange(len(wav_sorted_list) / batch_size+1):
        ...
        for j in xrange(len(index)):
            ...
            if j==0:
                max_len=len(audio)
                total_max_len+=__round_to(max_len,seq_len)
    ...
    if gpu_index!=0:
        diff_len=total_len-total_max_len
        if diff_len!=0:
            wav_batches.append(np.zeros((1,diff_len),dtype='float32'))
            mask_batches.append(np.zeros((1,diff_len),dtype='int32'))
            con_batches.append(np.zeros((1,diff_len/con_frame_size*con_dim),dtype='float32'))

    return wav_batches,mask_batches,con_batches

def load_data(wav_sorted_list,con_sorted_list,batch_size,seq_len,con_frame_size,con_dim,q_levels,q_type,sample_rate,gpu_num,gpu_index):

    wav_batches,mask_batches,con_batches=make_batches(wav_sorted_list,con_sorted_list,batch_size,seq_len,con_frame_size,con_dim,sample_rate,gpu_num,gpu_index)
...
class AudioReader(object):
    ...
    def __init__(self,
        ...
        self.enqueue = self.queue.enqueue([self.wav_placeholder,self.mask_placeholder,self.con_placeholder,self.reset_batch_placeholder,
                                            self.end_batch_placeholder,self.end_epoch_placeholder])
